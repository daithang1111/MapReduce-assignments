...
public class FinalExam_Q3_A_2 extends Configured implements Tool {
  private static final Logger LOG = Logger.getLogger(FinalExam_Q3_A_2.class);

  private static class MyMapper extends Mapper<LongWritable, Text, PairOfInts, IntWritable> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException,
      ...
      for (int i = 1; i < term_count.length; i++) {
        KEY.set(origin, 1);
        VALUE.set(Integer.parseInt(term_count[i]));
        context.write(KEY, VALUE);

        for (int j = i + 1; j < term_count.length; j++) {
          KEY.set(Integer.parseInt(term_count[i]), 2);
          VALUE.set(Integer.parseInt(term_count[j]));
          context.write(KEY, VALUE);

          KEY.set(Integer.parseInt(term_count[j]), 2);
          VALUE.set(Integer.parseInt(term_count[i]));
          context.write(KEY, VALUE);
  ...
  private static class MyReducer extends
      Reducer<PairOfInts, IntWritable, IntWritable, ArrayListOfIntsWritable> {
    ...
    @Override
    public void reduce(PairOfInts key, Iterable<IntWritable> values, Context context)
      ...
      if (key.getRightElement() == 2) {
        while (iter.hasNext()) {
          twoHop.put(iter.next().get(), true);
        }
  ...
  public int run(String[] args) throws Exception {
    ...
    LOG.info("Tool name: " + FinalExam_Q3_A_2.class.getSimpleName());
    LOG.info(" - input path: " + inputPath);
    LOG.info(" - output path: " + outputPath);
    LOG.info(" - num reducers: " + reduceTasks);

    Job job = Job.getInstance(getConf());
    job.setJobName(FinalExam_Q3_A_2.class.getSimpleName());
    job.setJarByClass(FinalExam_Q3_A_2.class);
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new FinalExam_Q3_A_2(), args);
  }
}
