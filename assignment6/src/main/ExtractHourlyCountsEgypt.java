...
public class ExtractHourlyCountsEgypt extends Configured implements Tool {
  ...
  private static class MyMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
    ...
    private final static Pattern pattern = Pattern
        .compile("([Ee][Gg][Yy][Pp][Tt]|[Cc][Aa][Ii][Rr][Oo])");

    @Override
    public void map(LongWritable key, Text value, Context context) throws IOException,
        InterruptedException {
      String[] line = ((Text) value).toString().split("\\t");
      if (line.length >= 4) {// length of line =4
        Matcher matcher = pattern.matcher(line[3]);
        if (matcher.find()) {
          ...
          if (date.length >= 6) {
            ...
            if (date[1].equalsIgnoreCase("feb")) {
              month = 2;
            } else if (date[1].equalsIgnoreCase("jan")) {
              month = 1;
            }

            // filter date
            if (month == 1 || month == 2) {
              if ((month == 1 && day >= 23) || (month == 2 && day <= 8)) {
                String hour = date[3].split(":")[0];

                WORD.set(month + "/" + day + "\t" + hour);
